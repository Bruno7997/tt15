class Game {
 ...
 //Atualizando o gameState
  update(state) {
    ...
    if(state==1){this.updateRound(Round+1);console.log("B");this.r=1;round=new Rounds;console.log("C");round.a=qdm.toFixed(0);console.log("D");if(round.a>50){nl+=nl/2.25}}
...
var d = B.damage
   if(buls.length>0 && (buls[b]!=undefined && buls[b][0][0]==plr && buls[i][0][1]==PLR)){
    buls[b][1][0].sprite.position.x=x
    buls[b][1][0].sprite.position.y=y
    buls[b][1][0].sprite.velocityX=vx
    buls[b][1][0].sprite.velocityY=vy
    buls[b][1][0].d=d
   }
  else if(buls.length==0 || (buls[b]==undefined)){
    this.createBullet(x,y,vx,vy,d,plr,PLR)
  }
...
createBullet(X,Y,VX,VY,D,Index,Posi){
  var bul = new Bullet(X,Y,VX,VY,D,0)
  buls.push([[Index,Posi],[bul]])
  }
}
